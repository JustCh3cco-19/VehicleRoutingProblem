...
void aco_vrp_v2_run(int n, int K, int cap, int m, double **c, double alpha, double beta, double rho, double tau0, double Q, unsigned int seed, Solution *best_sol, double *best_cost) {
    ...
    #pragma omp parallel default(shared) proc_bind(close)
    {
        ...
        for (int iter = 0; iter < fixed_epochs; iter++) {
            ...
            #pragma omp master
            {
                if (iter > 0 && mpi_size > 1) {
                    MPI_Wait(&mpi_req, MPI_STATUS_IGNORE);
                    float inv = 1.0f / (float)mpi_size;
                    // Applica i feromoni ricevuti (media MPI)
            ...
            double iter_local_best = t_best_c;
            #pragma omp critical
            { if (iter_local_best < *best_cost) { 
                *best_cost = iter_local_best; solution_copy(best_sol, ws.thread_best);
            ...
            #pragma omp master
            if (mpi_size > 1) {
                MPI_Iallreduce(MPI_IN_PLACE, tau_mat->data, (int)total_elements, MPI_FLOAT, MPI_SUM, MPI_COMM_WORLD, &mpi_req);
            }
#endif
        }
#ifdef USE_MPI
        #pragma omp master
        if (mpi_req != MPI_REQUEST_NULL) MPI_Wait(&mpi_req, MPI_STATUS_IGNORE);
#endif
        v2_ws_free(&ws);
    }
    if (mpi_rank == 0) printf("V2 SSP Completion. Final: %.3f. Time: %.3fs\n", *best_cost, wall_time() - start_time);
