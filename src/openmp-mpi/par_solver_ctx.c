...
static int	par_solver_alloc_scores(t_par_solver_ctx *ctx)
{
	size_t	score_count;
	size_t	score_bytes;
	...
	return (ctx->score_mat != NULL);
}

static void	par_solver_init_runtime(t_par_solver_ctx *ctx)
{
...
static int	par_solver_alloc_deltas(t_par_solver_ctx *ctx)
{
	...
	return (ctx->rank_deltas != NULL);
}

int	par_solver_init(t_par_solver_ctx *ctx)
{
	if (!par_solver_alloc_scores(ctx))
		return (0);
	par_solver_init_runtime(ctx);
	if (!ctx->iter_best_sol_rank)
		return (0);
#ifdef USE_MPI
	par_async_init(&ctx->async_ctx, ctx->mpi_size);
#endif
	if (!par_solver_alloc_deltas(ctx))
		return (0);
	return (1);
}
