    ...
    for (int k = 0; k < MAX_CANDIDATES; ++k) {
        ...
        for (int j = 1; j <= n; ++j) {
            ...
            for (int prev = 0; prev < k; ++prev) {
                if (d_cand_list[i * MAX_CANDIDATES + prev] == j) {
                    already_cand = true; break;
                }
            }
            
            if (!already_cand) {
                float c = d_costs[i * (n + 1) + j];
                if (c < min_c) { min_c = c; min_node = j; }
            }
        }
        
        if (lane == 0) {
            d_cand_list[i * MAX_CANDIDATES + k] = min_node;
        }
    }
}
...
__global__ void kernel_construct_solutions(
    ...
    int words_per_mask = (n + 1 + 31) / 32;
    uint32_t* my_bitmask = &shared_mem[warp_in_block * (words_per_mask + K * 3)];
    ...
    while (unvisited > 0 && step < (n + 2 * K)) {
        int v_idx = step % K;
        if (my_fleet[v_idx].capacity <= 0.5f) {
            step++;
            continue;
        }

        int curr = my_fleet[v_idx].curr_node;
        int next_node = -1;
        ...
        if (next_node != -1) {
            if (lane == 0) {
                ...
                d_ant_routes[global_warp_id * (n + 2 * K) + step] = next_node;
            }
...
void launch_setup_aco(const float* d_costs, int* d_cand_list, int n, int threads_per_block) {
    kernel_setup_aco<<<n + 1, 32>>>(d_costs, d_cand_list, n);
}
