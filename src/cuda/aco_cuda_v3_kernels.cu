...
__device__ static float cuda_v3_rand01(unsigned int *state) {
  *state = (*state * 1664525u) + 1013904223u;
  return (float)(*state) / 4294967295.0f;
}

__device__ static int cuda_v3_visited_get(const uint64_t *visited, int node) {
  return (int)((visited[(unsigned int)node >> 6] >>
                ((unsigned int)node & 63u)) &
               1u);
}

__device__ static void cuda_v3_visited_set(uint64_t *visited, int node) {
  visited[(unsigned int)node >> 6] |=
      ((uint64_t)1u << ((unsigned int)node & 63u));
}

__device__ static float cuda_v3_warp_sum(float value) {
  for (int offset = CUDA_V3_WARP_SIZE / 2; offset > 0; offset >>= 1) {
    value += __shfl_down_sync(0xFFFFFFFFu, value, offset);
  }
  return value;
}

__device__ static float cuda_v3_warp_prefix_sum(float value, int lane) {
  for (int offset = 1; offset < CUDA_V3_WARP_SIZE; offset <<= 1) {
...
__device__ static int cuda_v3_select_from_candidates_warp(
    int current, const float *tau, const int *candidate_idx,
    const float *eta_beta, const uint64_t *visited, unsigned int *rng_state,
    CudaV3Params params, float *total_score_out) {
  int lane = threadIdx.x & (CUDA_V3_WARP_SIZE - 1);
  int side = params.n + 1;
  ...
  if (lane == 0) {
    threshold = cuda_v3_rand01(rng_state) * total;
  }
  threshold = __shfl_sync(0xFFFFFFFFu, threshold, 0);

  {
    float prefix = cuda_v3_warp_prefix_sum(score, lane);
...
__device__ static int cuda_v3_select_global_fallback_warp(
    int current, const float *costs, const float *tau, const uint64_t *visited,
    unsigned int *rng_state, CudaV3Params params, float *total_score_out) {
  int lane = threadIdx.x & (CUDA_V3_WARP_SIZE - 1);
  int side = params.n + 1;
  int chunks = (params.n + CUDA_V3_WARP_SIZE - 1) / CUDA_V3_WARP_SIZE;
  ...
  for (int chunk = 0; chunk < chunks; ++chunk) {
    int node = 1 + lane + chunk * CUDA_V3_WARP_SIZE;
    if (node <= params.n && !cuda_v3_visited_get(visited, node)) {
      float eta = 1.0f / (costs[current * side + node] + CUDA_V3_EPS);
      local_total +=
          powf(tau[current * side + node], params.alpha) * powf(eta, params.beta);
    }
  }

  total = cuda_v3_warp_sum(local_total);
  total = __shfl_sync(0xFFFFFFFFu, total, 0);
  if (total_score_out) {
    *total_score_out = total;
  }
  ...
  for (int chunk = 0; chunk < chunks; ++chunk) {
    ...
    chunk_sum = cuda_v3_warp_sum(score);
...
__device__ static int cuda_v3_select_next_customer_warp(
    int current, const float *costs, const float *tau, const int *candidate_idx,
    const float *eta_beta, const uint64_t *visited, unsigned int *rng_state,
    CudaV3Params params, float *total_score_out) {
  float candidate_total = 0.0f;
  int next = cuda_v3_select_from_candidates_warp(current, tau, candidate_idx,
  ...
  return cuda_v3_select_global_fallback_warp(current, costs, tau, visited,
                                             rng_state, params, total_score_out);
}

__device__ static float cuda_v3_score_depot_move(int current, const float *costs,
                                                 CudaV3Params params) {
  int side = params.n + 1;
  float eta = 1.0f / (costs[current * side + 0] + CUDA_V3_EPS);
  return params.depot_close_weight * powf(eta, params.beta);
}

__device__ static int cuda_v3_select_next_move_warp(
    ...
    unsigned int *rng_state, CudaV3Params params) {
  int lane = threadIdx.x & (CUDA_V3_WARP_SIZE - 1);
  float customer_total = 0.0f;
  int customer = cuda_v3_select_next_customer_warp(
  ...
  if (close_allowed) {
    depot_score = cuda_v3_score_depot_move(current, costs, params);
  }
  ...
  if (lane == 0) {
    threshold = cuda_v3_rand01(rng_state) * (customer_total + depot_score);
  }
  threshold = __shfl_sync(0xFFFFFFFFu, threshold, 0);
...
__global__ void kernel_build_candidate_lists_v3(const float *costs,
  ...
  int side = n + 1;
  int best_nodes[CUDA_V3_MAX_CANDIDATES];
  float best_costs[CUDA_V3_MAX_CANDIDATES];
  int t;
  int node;
  ...
  for (t = 0; t < cand_k; ++t) {
    ...
    if (best_nodes[t] > 0) {
      float eta = 1.0f / (best_costs[t] + CUDA_V3_EPS);
...
__global__ void kernel_construct_solutions_v3(
  ...
  int ant = global_thread / CUDA_V3_WARP_SIZE;
  int side = params.n + 1;
  ...
  for (int vehicle = 0; vehicle < params.K; ++vehicle) {
    ...
    while (1) {
      ...
      move = cuda_v3_select_next_move_warp(current_b, close_allowed_b, costs,
      ...
      if (lane == 0) {
        ...
        ++route_len;
        ++route_load;
        cuda_v3_visited_set(visited_row, move);
...
void launch_build_candidate_lists_v3(const float *costs, int *candidate_idx,
                                     ...
                                     float beta) {
  int total = n + 1;
  int blocks = (total + CUDA_V3_THREADS_PER_BLOCK - 1) /
               CUDA_V3_THREADS_PER_BLOCK;
  kernel_build_candidate_lists_v3<<<blocks, CUDA_V3_THREADS_PER_BLOCK>>>(
...
void launch_init_tau_v3(float *tau, int n, float tau0) {
  ...
  int blocks = (total + CUDA_V3_THREADS_PER_BLOCK - 1) /
               CUDA_V3_THREADS_PER_BLOCK;
  kernel_init_tau_v3<<<blocks, CUDA_V3_THREADS_PER_BLOCK>>>(tau, n, tau0);
}
...
void launch_reset_ant_state_v3(int *routes, int *route_lengths,
  ...
  int blocks =
      (params.m + CUDA_V3_THREADS_PER_BLOCK - 1) / CUDA_V3_THREADS_PER_BLOCK;
  kernel_reset_ant_state_v3<<<blocks, CUDA_V3_THREADS_PER_BLOCK>>>(
...
void launch_construct_solutions_v3(const float *costs, const float *tau,
                                   ...
                                   CudaV3AntSummary *ant_summary,
                                   CudaV3Params params) {
  int warps_per_block = CUDA_V3_THREADS_PER_BLOCK / CUDA_V3_WARP_SIZE;
  int blocks = (params.m + warps_per_block - 1) / warps_per_block;
  kernel_construct_solutions_v3<<<blocks, CUDA_V3_THREADS_PER_BLOCK>>>(
...
void launch_evaporate_tau_v3(float *tau, int n, float rho) {
  ...
  int blocks = (total + CUDA_V3_THREADS_PER_BLOCK - 1) /
               CUDA_V3_THREADS_PER_BLOCK;
  kernel_evaporate_tau_v3<<<blocks, CUDA_V3_THREADS_PER_BLOCK>>>(tau, n, rho);
}
...
void launch_deposit_solution_v3(float *tau, const int *routes,
  ...
  int blocks =
      (K + CUDA_V3_THREADS_PER_BLOCK - 1) / CUDA_V3_THREADS_PER_BLOCK;
  kernel_deposit_solution_v3<<<blocks, CUDA_V3_THREADS_PER_BLOCK>>>(
...
void launch_clamp_tau_v3(float *tau, int n, float tau_min, float tau_max) {
  ...
  int blocks = (total + CUDA_V3_THREADS_PER_BLOCK - 1) /
               CUDA_V3_THREADS_PER_BLOCK;
  kernel_clamp_tau_v3<<<blocks, CUDA_V3_THREADS_PER_BLOCK>>>(tau, n, tau_min,
                                                             tau_max);
}
...
void launch_recenter_tau_v3(float *tau, int n, float tau0) {
  ...
  int blocks = (total + CUDA_V3_THREADS_PER_BLOCK - 1) /
               CUDA_V3_THREADS_PER_BLOCK;
  kernel_recenter_tau_v3<<<blocks, CUDA_V3_THREADS_PER_BLOCK>>>(tau, n, tau0);
}
