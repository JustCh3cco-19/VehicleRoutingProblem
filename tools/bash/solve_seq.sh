...
if [ ! -f "$manifest" ]; then
  echo "[ERROR] sequential manifest not found: $manifest" >&2
  exit 2
fi
if [ ! -x ./aco_vrp_seq.out ]; then
  echo "[ERROR] sequential executable not found: ./aco_vrp_seq.out (run 'make seq')" >&2
  exit 2
fi
...
if [[ ! "$runtime_s" =~ ^[1-9][0-9]*$ ]] || [ "$runtime_s" -gt 300 ]; then
  echo "[ERROR] SOLVE_SEQ_RUNTIME_S must be an integer between 1 and 300" >&2
  exit 2
fi
if [[ ! "$stag_iters" =~ ^[0-9]+$ ]]; then
  echo "[ERROR] SOLVE_SEQ_STAGNATION_EPOCHS must be a non-negative integer" >&2
  exit 2
fi
if [[ ! "$improve_rel_pct" =~ ^[0-9]+([.][0-9]+)?$ ]] || awk "BEGIN {exit !($improve_rel_pct <= 0)}"; then
  echo "[ERROR] SOLVE_SEQ_MIN_REL_IMPROVEMENT must be a positive number" >&2
  exit 2
fi
if [[ ! "$repeats" =~ ^[1-9][0-9]*$ ]]; then
  echo "[ERROR] SOLVE_SEQ_REPEATS must be a positive integer" >&2
  exit 2
fi
if [[ ! "$limit" =~ ^[0-9]+$ ]]; then
...
if [[ ! "$seq_m" =~ ^[0-9]+$ ]]; then
  echo "[ERROR] SOLVE_SEQ_M must be a non-negative integer" >&2
  exit 2
fi
...
if [ -s "$csv" ] && [ "$(head -n1 "$csv")" != "$header" ]; then
  echo "[ERROR] incompatible sequential CSV header: $csv" >&2
  exit 2
fi
...
tail -n +2 "$manifest" \
  ...
  | while IFS=, read -r profile name instance_path n K m solver_seed instance_seed layout_id capacity_formula; do
      ...
      for ((run_id = 1; run_id <= repeats; run_id++)); do
        ...
        echo "[seq][avvio] istanza=${name} | run=${run_id} | avanzamento_campagna=${done_runs}/${total_runs}"
        echo "[seq][limiti] tempo_massimo=${runtime_s}s | epoche_stagnazione=${stag_iters} | durata_stimata=${eta_run_s}s | memoria_stimata=${eta_mem_gb}GiB"
        ...
        if [ "$rc" -eq 0 ]; then
          echo "[seq][completato] tempo_trascorso=${elapsed:-n/a}s | tempo_rimanente=0s | best_cost=${cost:-n/a} | stato=ok | memoria=${rss_gb:-n/a}GiB"
        else
          echo "[seq][completato] tempo_trascorso=${elapsed:-n/a}s | tempo_rimanente=n/a | best_cost=n/a | stato=errore | memoria=${rss_gb:-n/a}GiB"
        fi
        echo
      done
    done
