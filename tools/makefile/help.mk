help:
	...
	@printf "  make solve_cuda SOLVE_CLIENTS=500,1000 SOLVE_CUDA_REPEATS=3 SOLVE_SEQ_RUNTIME_S=300\n"
	@printf '%s\n' '  cores=$$(lscpu -p=Core | grep -v '"'"'^#'"'"' | sort -u | wc -l); threads=$$(( $$(nproc) / cores )); make solve_mpi SOLVE_CLIENTS=4000,8000 SOLVE_MPI_RANKS=$$cores SOLVE_MPI_OMP_THREADS=$$threads'
	@printf "  make solve_seq && make solve_mpi && make solve_cuda\n\n"
	@printf "COMANDI ESEMPIO - CLUSTER (SLURM):\n"
	@printf "  tools/batch/submit_solve.sh --target solve_seq --make-args \"SOLVE_SEQ_REPEATS=3 SOLVE_SEQ_RUNTIME_S=300\"\n"
	@printf "  tools/batch/submit_solve.sh --target solve_mpi --make-args \"SOLVE_MPI_REPEATS=3 SOLVE_MPI_RUNTIME_S=300 SOLVE_MPI_RANKS=4 SOLVE_MPI_OMP_THREADS=8\"\n"
	@printf "  tools/batch/submit_solve.sh --target solve_cuda --partition gpu --gres gpu:1 --make-args \"SOLVE_CUDA_REPEATS=3 SOLVE_SEQ_RUNTIME_S=300 SOLVE_CUDA_VARIANT=v6 CUDA_ARCH=sm_75\"\n"
