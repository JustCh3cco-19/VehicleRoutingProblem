...
#define L1_MAX_CANDS 512
#define L2_MAX_CANDS 2048
#define V2_EPS 1e-7f
...
static int v2_shared_init(V2RankShared *s, int n, int l1_k, int l2_k, const MatrixFloat *c_mat, double beta) {
    s->n = n; s->l1_k = l1_k; s->l2_k = l2_k;
    size_t row_bytes = (size_t)s->l2_k * sizeof(int);
    s->stride = (int)(align_up_64(row_bytes) / sizeof(int));
    ...
    for (int i = 0; i <= n; i++) {
        int nodes[L2_MAX_CANDS]; float dists[L2_MAX_CANDS];
        for (int t = 0; t < s->l2_k; t++) { nodes[t] = -1; dists[t] = FLT_MAX; }
        const float *row = c_mat->rows[i];
        for (int node = 1; node <= n; node++) {
            if (node == i) continue;
            float d = row[node]; int pos = -1;
            for (int t = 0; t < s->l2_k; t++) { if (d < dists[t]) { pos = t; break; } }
            if (pos >= 0) {
                for (int m = s->l2_k - 1; m > pos; m--) { dists[m] = dists[m-1]; nodes[m] = nodes[m-1]; }
                dists[pos] = d; nodes[pos] = node;
            }
        }
        int *c_row = s->cand_idx + (size_t)i * (size_t)s->stride;
        float *e_row = s->eta_beta + (size_t)i * (size_t)s->stride;
        for (int t = 0; t < s->l2_k; t++) {
            c_row[t] = (nodes[t] > 0) ? nodes[t] : 0;
            // Calcoliamo eta_beta solo per L1 per risparmiare memoria/L3
            if (t < s->l1_k) e_row[t] = (nodes[t] > 0) ? fast_powf(1.0f / (dists[t] + 1e-7f), (float)beta) : 0.0f;
        }
    }
    return 1;
}
...
static int find_nearest_unvisited(const V2RankShared *s, int curr, const uint64_t *visited, const MatrixFloat *c) {
    ...
    for (int w = 0; w < s->visited_words; w++) {
        ...
        while (mask != 0) {
            ...
            float d = row[node];
            if (d < best_d) { best_d = d; best = node; }
            mask &= mask - 1;
        }
    }
    return best;
...
static void build_ant_v2_two_level(AcoThreadWorkspace *ws, const V2RankShared *s, int K, int cap, const MatrixFloat *c, const float *scores) {
    ...
    for (int v = 0; v < K; v++) {
        ...
        while (true) {
            ...
            int next = 0;
            // LIVELLO 1: Probabilistico (Top 512)
            float denom = 0.0f;
            const int *cands = s->cand_idx + (size_t)curr * (size_t)s->stride;
            const float *sc = scores + (size_t)curr * (size_t)s->l1_k; // Nota: score_mat è denso L1
            float weights[L1_MAX_CANDS];
            for (int t = 0; t < s->l1_k; t++) {
                int node = cands[t];
            ...
            if (denom > 0.0f) {
                ...
                for (int t = 0; t < s->l1_k; t++) { if (weights[t] <= 0.0f) continue; cum += weights[t]; if (cum >= thres) { next = cands[t]; break; } }
            }

            // LIVELLO 2: Deterministico (Nearest in Top 2048)
            if (next <= 0) {
                for (int t = s->l1_k; t < s->l2_k; t++) {
                    int node = cands[t];
                    if (node > 0 && !((ws->visited[(unsigned)node >> 6] >> ((unsigned)node & 63u)) & 1u)) {
                        next = node; break; // Il primo unvisited è il più vicino
                    }
                }
            }

            // LIVELLO 3: Fallback O(N)
...
void aco_vrp_v2_run(int n, int K, int cap, int m, double **c, double alpha, double beta, double rho, double tau0, double Q, unsigned int seed, Solution *best_sol, double *best_cost) {
    ...
    V2RankShared shared; v2_shared_init(&shared, n, l1_k, l2_k, c_mat, beta);
    
    // score_mat memorizza solo L1 per risparmiare spazio in cache
    float *score_mat = aligned_alloc(V2_ALIGNMENT, (size_t)(n + 1) * (size_t)shared.l1_k * sizeof(float));
    ...
    #pragma omp parallel default(shared) proc_bind(close)
    {
        ...
        for (int iter = 0; iter < fixed_epochs; iter++) {
            ...
            for (int i = 0; i <= n; i++) {
                ...
                float *sc = score_mat + (size_t)i * (size_t)shared.l1_k; const float *tau_row = tau_mat->rows[i];
                for (int t = 0; t < shared.l1_k; t++) { int node = cands[t]; sc[t] = (node > 0) ? (fast_powf(tau_row[node], (float)alpha) * etas[t]) : 0.0f; }
            }
            #pragma omp barrier
            ...
            for (int a = 0; a < local_m; a++) {
                ...
                build_ant_v2_two_level(&ws, &shared, K, cap, c_mat, score_mat);
            ...
            #pragma omp barrier
            // Pheromone update logic omitted for benchmarking construction speed
        }
        v2_ws_free(&ws);
    }
    if (mpi_rank == 0) printf("V2 Two-Level Candidates Completion. Time: %.3fs\n", wall_time() - start_time);
