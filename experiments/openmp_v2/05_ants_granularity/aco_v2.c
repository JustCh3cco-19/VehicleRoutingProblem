...
/* -- Helper Statici (Ottimizzati per V2) -- */
...
static int tune_candidate_k(int n, long l3_size) {
    if (l3_size <= 0) return 32; // Fallback
    // Target: score_mat (4 bytes/elem) occupies 60% of L3
...
static void load_v2_directives(double *max_runtime_sec, int *max_stagnation_epochs, double *min_rel_improvement) {
  ...
  *min_rel_improvement = (s_rel && *s_rel) ? atof(s_rel) : 1e-3;
  ...
  if (*max_stagnation_epochs < 0) *max_stagnation_epochs = 0;
  if (*min_rel_improvement <= 0.0) *min_rel_improvement = 1e-3;
}
...
/* -- Strutture Dati Private (Workspace Isolato) -- */
...
static int v2_shared_init(V2RankShared *s, int n, int cand_k, const Matrix *c_mat, double beta) {
    ...
    for (int i = 0; i <= n; i++) {
        int nodes[V2_MAX_CANDS]; double dists[V2_MAX_CANDS];
        for (int t = 0; t < s->cand_k; t++) { nodes[t] = -1; dists[t] = DBL_MAX; }
        ...
        for (int t = 0; t < s->cand_k; t++) {
            ...
            e_row[t] = (nodes[t] > 0) ? (float)fast_pow(1.0 / (dists[t] + 1e-9), beta) : 0.0f;
        }
    }
    return 1;
}
        ...
        while (rem > 0 && rem > fut_cap && ws->route_loads[v] < cap) {
            ...
            for (int t = 0; t < s->cand_k; t++) {
                ...
                if (node > 0 && !((ws->visited[(unsigned)node >> 6] >> ((unsigned)node & 63u)) & 1u)) {
                    weights[t] = sc[t]; denom += (double)sc[t];
                } else weights[t] = 0.0f;
            }
            if (denom > 0.0) {
...
void aco_vrp_v2_run(int n, int K, int cap, int m, double **c, double alpha, double beta, double rho, double tau0, double Q, unsigned int seed, Solution *best_sol, double *best_cost) {
    ...
    long l3_size = get_l3_cache_size();
    int cand_k = tune_candidate_k(n, l3_size);
    ...
    #pragma omp parallel for collapse(2) schedule(static)
    for (int i = 0; i <= n; i++) {
        for (int j = 0; j <= n; j++) tau_mat->rows[i][j] = (i==j) ? 0.0 : tau0;
    }

    V2RankShared shared; if (!v2_shared_init(&shared, n, cand_k, c_mat, beta)) return;
    ...
    *best_cost = DBL_MAX; double start_time = wall_time();
    int iter_since_best = 0, total_iters = 0;
    bool stop_now = false;
    ...
    if (mpi_rank == 0) {
        printf("Hybrid V2 Solver starting... (N=%d, K=%d, M=%d)\n", n, K, total_m);
        ...
        else printf("  L3 Cache detection failed. Using default K_cand = %d\n", cand_k);
    ...
    #pragma omp parallel default(shared) proc_bind(close)
    {
        ...
        for (int iter = 0;; iter++) {
            ...
            {
              total_iters++;
              double current_time = wall_time();
              if (fixed_epochs > 0) {
                  if (iter >= fixed_epochs) stop_now = true;
              } else {
                  if (max_runtime_sec > 0.0 && (current_time - start_time) > max_runtime_sec) stop_now = true;
            ...
            for (int i = 0; i <= n; i++) {
                ...
                for (int t = 0; t < shared.cand_k; t++) {
                    int node = cands[t];
                    sc[t] = (node > 0) ? (float)(fast_pow(tau_row[node], alpha) * etas[t]) : 0.0f;
                }
            }
            #pragma omp barrier

            // 2. Ant Construction Dynamic/Guided
            ...
            #pragma omp master
            {
                ...
                if (iter_best_cost < *best_cost) {
                    if (is_sig_imp(*best_cost, iter_best_cost, min_rel_improvement)) {
                        iter_since_best = 0;
                    } else {
                        iter_since_best++;
                    }
                    *best_cost = iter_best_cost; solution_copy(best_sol, iter_best);
                    tau_max = 1.0 / ((1.0 - rho) * (*best_cost)); tau_min = tau_max * 0.05;
            ...
            for (int i = 0; i <= n; i++) {
                for (int j = 0; j <= n; j++) if (i != j) tau_mat->rows[i][j] *= (1.0 - rho);
            }
            ...
            // Stagnation Reset
            bool reset_needed = (iter_since_best > 0 && (iter_since_best % 32 == 0));
            if (reset_needed) {
              #pragma omp for schedule(runtime)
              for (int i = 0; i <= n; i++) {
                  for (int j = 0; j <= n; j++) {
    ...
    matrix_free_handle(tau_mat); matrix_free_handle(c_mat);
