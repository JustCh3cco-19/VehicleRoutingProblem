...
mpicc -Wall -Wextra -std=c11 -Iinclude -O3 -fopenmp -DUSE_MPI -DACO_VRP_V2 \
    src/main.c \
    experiments/openmp_v2/07_performance_breakdown/aco_v2_profiled.c \
    src/common/aco_shared.c src/common/solution.c src/common/matrix.c src/common/instance_parser.c \
    -lm -o experiments/openmp_v2/07_performance_breakdown/aco_double.out

# 2. Compile and Run FLOAT version
echo "Compiling profiled V2 (FLOAT)..."
mpicc -Wall -Wextra -std=c11 -Iinclude -O3 -fopenmp -DUSE_MPI -DACO_VRP_V2 \
    src/main.c \
    experiments/openmp_v2/07_performance_breakdown/aco_v2_float.c \
    src/common/aco_shared.c src/common/solution.c src/common/matrix.c src/common/instance_parser.c \
    -lm -o experiments/openmp_v2/07_performance_breakdown/aco_float.out
...
echo "===================================================="
./experiments/openmp_v2/07_performance_breakdown/aco_double.out "$INSTANCE" "$K" "$M" "$SEED" | grep -A 10 "V2 PERFORMANCE"

echo ""
echo "===================================================="
echo "RUNNING FLOAT VERSION"
echo "===================================================="
./experiments/openmp_v2/07_performance_breakdown/aco_float.out "$INSTANCE" "$K" "$M" "$SEED" | grep -A 10 "V2 FLOAT PERFORMANCE"
